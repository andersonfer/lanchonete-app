...
# RDS MySQL - Cozinha
resource "aws_db_instance" "mysql_cozinha" {
  identifier = "${local.prefix}-cozinha-mysql"
  ...
  db_name  = "cozinha_db"
  username = "admin"
  password = random_password.mysql_cozinha.result
  ...
  tags = merge(
    local.common_tags,
    {
      Name    = "${local.prefix}-cozinha-mysql"
      Service = "cozinha"
    }
  )
}
